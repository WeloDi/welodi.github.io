...
### 3.3 VTS/HTS：帧率与曝光的约束

> PLL 只决定 pixel_rate（每秒输出多少像素），而它如何被切成行/帧、曝光最长能开多久，由 HTS/VTS 这两个帧时序寄存器决定。

**曝光以行周期为单位：**
- 曝光时间（Integration Time）按行设置：`曝光时间 = 曝光行数 × line_time`，行周期 `line_time = HTS / pixel_rate`。
- 因此曝光的最小步进 = 一个行周期：HTS 越小行周期越短，曝光控制粒度越细、精度越高。

**一帧曝光必须装进 VTS 的"盒子"里：**
- 整帧的曝光 + 读出要容纳在 VTS 定义的帧周期内，故最大曝光行数被 VTS 严格限制：`EXPO_MAX = VTS - 安全余量`。
- 安全余量是固定偏移，用于保证上一帧读出与电路复位来得及：IMX219 为 4 行（见 6.5），OV9281 为 25 行。

**帧率 = pixel_rate / (VTS × HTS)，两个调节旋钮：**
- 调 **VTS**（常用）：V_Blank 不参与成像，加大 VTS 即拉长帧周期降低帧率、为长曝光留出空间，减小则提高帧率。
- 调 **HTS**（谨慎）：加大 H_Blank 同样降低帧率，但它挤占曝光时间、并改变曝光计算的基础单位（行周期），牵一发动全身，一般不作为调帧率的手段。
